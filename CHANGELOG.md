...
- **CI verify gate before image push** (#24, roadmap O5). Both `deploy-main.yml` and `deploy-dev.yml` now run a `verify` job (`npm ci` → `prisma generate` → `lint` → `build` → `test`) before the `build-push` job. If verify fails, `build-push` is skipped — `:latest` and `:dev` are never advanced past a known-bad commit. Branch protection's required-check on `build-push` transitively requires verify too, so no protection-rule changes are needed.
